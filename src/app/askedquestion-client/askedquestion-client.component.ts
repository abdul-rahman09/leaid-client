...
export class AskedquestionClientComponent implements OnInit {
	...
	updateReply() {
		...
		this.http.post("https://leaid.herokuapp.com/askedquestion/",formdata).toPromise().then((res:any) => {
			this.askedQuestions = res;
			for (let i = 0; i < res.length; ++i) {
				let parameter = {
					'question': res[i].id
				}
				this.http.post("https://leaid.herokuapp.com/getreply/",parameter).subscribe((reply:any) => {
