...
export class ReplyquestionLawyerComponent implements OnInit {
	...
	updateReply() {
		this.data = this.authService.getUserData()
			...
			this.http.post("https://leaid.herokuapp.com/askedquestionLawyer/",formdata).toPromise().then((res:any) => {
				this.askedQuestions = res;
				for (let i = 0; i < res.length; ++i) {
					let parameter = {
						'question': res[i].id
					}
					this.http.post("https://leaid.herokuapp.com/getreply/",parameter).toPromise().then((reply:any) => {
	...
	reply(question) {
		...
		this.http.post("https://leaid.herokuapp.com/replyquestion/",parameter).toPromise().then((reply:any) => {
			this.text = ""
