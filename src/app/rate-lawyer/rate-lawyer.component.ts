...
export class RateLawyerComponent implements OnInit {
  ...
  ngOnInit() {
	...
	this.http.post("https://leaid.herokuapp.com/getLawyerById/",id).subscribe((lawyer:any) => {
	...
	onSubmit() {
		...
		this.http.post("https://leaid.herokuapp.com/RateLawyer/",formdata).subscribe((res:any) => {
			this.rate = 4;
			this.title = "";
			this.description = "";
